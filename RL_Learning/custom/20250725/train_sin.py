...
# 创建环境
def make_env():
    env = gym.make('SinEnv-v0', render_mode=None)
    return Monitor(env)
...
eval_callback = EvalCallback(
    Monitor(gym.make('SinEnv-v0')),
    best_model_save_path="./best_model/",
    log_path="./logs/",
    eval_freq=50000,  # 增加评估频率间隔
    deterministic=True,
    verbose=1
)
...
early_stop_callback = EarlyStoppingCallback(
    ...
    check_freq=10000        # 每10000步检查一次，适应更长的episode
)

# 初始化模型（使用CPU避免GPU相关问题）
model = PPO(
    "MlpPolicy",
    vec_env,
    ...
    verbose=1,
    learning_rate=1e-4,
    n_steps=32768,  # 增加到与max_steps相同，确保能收集完整episode
    batch_size=512,  # 相应增加batch_size
    n_epochs=20,
    gamma=0.99,
    gae_lambda=0.95,
    clip_range=0.2,
