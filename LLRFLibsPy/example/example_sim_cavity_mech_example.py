...
for i in range(sim_len):
    ...
    # piezo compensation
    piezo_peak = 2.0 * np.pi * 500
    piezo_length = 750
    piezo_pulse = np.zeros(sim_len, dtype = np.float64)
    dw_piezo = piezo_peak * base_pul[buf_id if buf_id < len(piezo_pulse) else -1]

    # cavity
    vc, vr, dw, state_vc, state_m = sim_cav(wh, RL, dw, dw0 + dw_micr + dw_piezo, S2, state_vc, Ts,
                                            beta        = beta,
                                            state_m0    = state_m, 
                                            Am          = Ad, 
                                            Bm          = Bd, 
